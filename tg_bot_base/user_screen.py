...
class UserScreen:
    ...
    def clear(self, user_id: int):
        user_data = self.user_data_manager.get(user_id)
...
new_screen must be not None here"""
    ...
    async def set_by_name(self, user_id: int, screen_name: str):
        directory_stack = self.user_data_manager.get(user_id).directory_stack
    ...
    def _get(self, user_id: int) -> EvaluatedScreen | None:
        ...
        screen: EvaluatedScreen = self.user_data_manager.get(user_id).screen
        if screen is None:
            return None
        return screen.clone()
